(function (win) {
    ...
    vTools.Animals = function (voption) {
        ...
        var currentData = null, currentIndex = 0, currentDom = null, idPrefix = 'animal-id-', option = { filter: "html", data: [] }, vself = this;

        option = $.extend(option, voption);

        //排序
        var vData = option.data == undefined ? [] : option.data;
        ...
        var _RenderAnimal = function (dom, animatedArry, callback) {
            ...
            _raself._ShowAnimal = function (showIndex) {
                ...
                //渲染添加动画样式
                dom.addClass("animated " + animatedArry[showIndex].name).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function () {
        ...
        //开始执行
        vself.Start = function () {

            //初始加载数据
            vTools.GetDomData(vData, $(option.filter+":first").find("*[data-animal]"), "data-animal", idPrefix);

            //排序
            vData = vTools.Sort(vData, "animalsort");
